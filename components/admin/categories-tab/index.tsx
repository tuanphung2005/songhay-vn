        ...
        <Card className="border-zinc-200 shadow-sm">
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-base">Tạo chuyên mục</CardTitle>
          ...
          <CardContent className="pt-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg bg-white p-3 text-center shadow-sm ring-1 ring-zinc-200">
                <p className="text-2xl font-bold text-zinc-800">{rootCategories.length}</p>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-base">Tất cả chuyên mục</CardTitle>
                <CardDescription className="text-xs">
                  {categoriesForManage.length} chuyên mục · Hover để sắp xếp · ✏️ để sửa
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="space-y-3">
          {categoriesForManage.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-12 text-center">
              ...
              <p className="text-sm font-medium text-zinc-600">Chưa có chuyên mục nào</p>
              <p className="text-xs text-zinc-400">Tạo chuyên mục đầu tiên từ form bên trái.</p>
            </div>
          ) : (
            rootCategories.map((root, ri) => {
              const children = categoriesForManage.filter((c) => c.parentId === root.id)
              return (
                <CategoryGroup
                  key={root.id}
                  parent={root}
