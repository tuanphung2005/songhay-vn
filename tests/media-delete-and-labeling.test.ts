...
describe("media delete and labeling", () => {
  ...
  test("media library UI has filter/search and pagination controls", () => {
    ...
    expect(source).toContain('const initialFilterType: "image" | "video" = "image"')
    expect(source).toContain('const [filterType, setFilterType] = useState<"image" | "video">(initialFilterType)')
